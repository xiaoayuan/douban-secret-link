...
export default function RegisterPage() {
  ...
  const [doubanName, setDoubanName] = useState("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ doubanUid, doubanName, password }),
      });
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={doubanUid}
              onChange={(e) => setDoubanUid(e.target.value)}
              placeholder="你的豆瓣UID（小组成员页中的ID）"
          ...
          <div>
            ...
            <input
              type="text"
              value={doubanName}
              onChange={(e) => setDoubanName(e.target.value)}
              placeholder="你的豆瓣昵称"
              className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-gray-500 focus:outline-none"
              required
            />
        ...
        <p className="mt-4 text-center text-sm text-gray-600">
          已有账号？
          <Link href="/login" className="text-gray-900 underline">登录</Link>
        </p>
      </div>
    </div>
  );
