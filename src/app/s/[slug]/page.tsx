...
export default function SecretPage() {
  ...
  const startCommentChallenge = async () => {
    if (!doubanUid.trim()) return;
    setAuthChecking(true); setError("");
    try {
  ...
  const checkCommentChallenge = async () => {
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#f6f1e8] px-4 py-10">
      <main className="mx-auto max-w-md">
        <div className="rounded-[28px] border border-[#e3d8c5] bg-[#fffaf1] p-6 shadow-[0_24px_80px_rgba(64,48,28,0.12)]">
          <div className="mb-6 text-center">
            ...
            <p className="mt-3 text-sm leading-6 text-[#6f604d]">这是仅限小组成员查看的私密内容。首次访问需要去豆瓣验证帖回复一次性口令，用来确认你控制这个豆瓣账号。</p>
          </div>
          ...
          {!challengeCode ? (
            <div className="space-y-3">
              ...
              <button onClick={startCommentChallenge} disabled={authChecking || !doubanUid.trim()} className="w-full rounded-2xl bg-[#244b36] py-3 text-sm font-semibold text-white disabled:opacity-50">{authChecking ? "生成中..." : "生成验证口令"}</button>
            ...
            <div className="space-y-4">
              ...
              <button onClick={checkCommentChallenge} disabled={authChecking} className="w-full rounded-2xl bg-[#244b36] py-3 text-sm font-semibold text-white disabled:opacity-50">{authChecking ? "检查中..." : "我已回复，开始验证"}</button>
